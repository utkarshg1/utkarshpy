...
class TestVirtualEnv:
    ...
    def test_setup_virtualenv_with_template_py(
        self, temp_project_dir, mock_file_exists, monkeypatch, capsys
    ):
        """Test setup_virtualenv when template.py exists."""
        # Mock pyproject.toml and template.py
        mock_file_exists.add("pyproject.toml")
        mock_file_exists.add("template.py")
        ...
        def track_command(cmd, **kwargs):
            commands.append(cmd)
            if "uv venv" in cmd or "uv run template.py" in cmd:
                return None  # Simulate successful execution
            if kwargs.get("live_output"):
                kwargs["live_output"] = False  # Disable live output for testing
            return orig_run_command(cmd, **kwargs)
        ...
        # Verify template.py was run with proper command
        assert any("uv run template.py" in cmd for cmd in commands)

        out = capsys.readouterr().out
        assert "Running template.py to generate extra folders and files" in out
        assert "Extra folders and files generated by template.py" in out

        # Test Linux platform
        monkeypatch.setattr(sys, "platform", "linux")
        commands.clear()  # Clear commands for the new platform
        ...
        # Check for bash-specific command formation for template.py
        template_cmd_found = False
        for cmd in commands:
            if "bash -c" in cmd and "uv run template.py" in cmd:
                template_cmd_found = True
                break
        assert template_cmd_found, "Template.py wasn't run with bash -c on Linux"

        out = capsys.readouterr().out
        assert "Running template.py to generate extra folders and files" in out
        assert "Extra folders and files generated by template.py" in out

    def test_setup_virtualenv_without_template_py(
        self, temp_project_dir, mock_file_exists, monkeypatch, capsys
    ):
        """Test setup_virtualenv when template.py does not exist."""
        # Mock pyproject.toml but not template.py
        mock_file_exists.add("pyproject.toml")
        ...
        # Verify template.py was NOT run
        for cmd in commands:
            assert (
                "uv run template.py" not in cmd
            ), "template.py was run but shouldn't be"

        # Check output - should not contain template.py messages
        out = capsys.readouterr().out
        assert "Running template.py" not in out
        assert "Extra folders and files generated by template.py" not in out

    def test_setup_virtualenv_template_py_failure(
        self, temp_project_dir, mock_file_exists, monkeypatch, mock_subprocess
    ):
        """Test handling of template.py execution failure."""
        # Mock pyproject.toml and template.py
        mock_file_exists.add("pyproject.toml")
        mock_file_exists.add("template.py")

        # Mock run_command to throw the correct error for template.py
        original_run_command = cli.run_command

        def mock_run_cmd(cmd, **kwargs):
            if "uv venv" in cmd:
                # Let the virtual environment creation succeed
                return None
            elif "template.py" in cmd:
                # Make template.py execution fail with a SystemExit
        ...
        # Run setup_virtualenv and expect SystemExit
        with pytest.raises(SystemExit):
            cli.setup_virtualenv()
...
class TestMainFunction:
    ...
    def test_main_happy_path(self, monkeypatch, capsys):
        ...
        def fake_create_github_repo(name, visibility, push=True):
            ...
            # Force exit here to trigger the SystemExit exception
            sys.exit(0)
            return "https://github.com/test/test"
